...
export function isValid(question: Question, answer: string): boolean {
    const isShortAnswer: boolean = question.type == "short_answer_question";

    /**
     * Nested ternary if: checks if question is short answer;
     * if not, checks if answer is in options array
     */
    return (
        isShortAnswer ? true
...
export function toMarkdown(question: Question): string {
    /**
     * New options list to add hyphen to every item, line break to every but last item
     */
    const options: string[] = question.options.map((option: string): string =>
...
export function addOption(question: Question, newOption: string): Question {
    /**
     * New options list to avoid referencing existing list from `question`
     */
    const options = question.options.map((option: string): string => option);
    options.splice(options.length, 0, newOption);
